...
export class JobDashboardComponent {
  ...
  apply(job: JobResponse) {
    if (!this.resumeFor(job.id)) { this.showToast('error', 'Add a resume before applying.'); return; }
    this.applied.update((s) => new Set(s).add(job.id));
    this.showToast('success', `Applied to ${job.jobTitle} at ${job.companyName}`);
    // NOTE: persisting the application to the DB arrives with the Application Management feature.
  }
